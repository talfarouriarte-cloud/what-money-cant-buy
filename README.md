...
- **v3.5.3** (Apr 2026): Updated Tracker About-this-tab text (EN+ES) to fully describe the three lines in Position view: solid actual line, lighter p50 projection, and orange dashed final-rank evolution. Multi-team explanation now also reflects per-team actual + lighter projection lines and notes that the orange line is hidden in multi mode.
- **v3.5.2** (Apr 2026): Added missing About-this-tab explanations. Home: new entry for the "Next matchday" card (`card_matches`), describing the closest and most-uneven fixtures of the upcoming league round. Run Evaluator: new paragraph documenting the match-by-match ranking table shown below the chart (chronological list of every match in the selected range with actual/expected/diff). 4 new i18n keys (EN+ES). SW v77.
- **v3.5.1** (Apr 2026): Fixed gap between actual and projected lines in Tracker Position view. The last played gameweek now writes both the `act` and `proj` data fields with the same value, so Recharts draws the projection line as a continuous extension of the actual line. SW v76.
