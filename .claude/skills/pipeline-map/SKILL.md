...
Cambios de framework 2026-07-12 (process-proposals del repo de origen; entonces vía vendored sync — desde 2026-07-13 la capa vendored se injerta en runtime desde agent-pipeline@main, AP-009, sin copias ni sync): Reviewer jamás da LGTM sobre CI completado en rojo (#1269); Creator recupera literales must-copy del body RAW y publica huella `pre-reviewer:` en cada PR (#1278/#1259); Auditor aterriza el archivo de epic-context directamente en la rama por defecto (#1277); Watchdog con fast-path de CI rojo atribuible, marcador `watchdog-ci-attributable` (#1268); process-reviewer publica-antes-de-commitear con chequeo de integridad y señal determinista de fallo (#1258); guard de panel en claude-code: el arm manual de `epica` exige panel de la épica anterior consumido (override `<!-- panel-ok -->`).
